...
export default function ContentPage() {
  ...
  useEffect(() => { const raw = window.localStorage.getItem(STORAGE_KEY); if (!raw) return; try { const parsed = JSON.parse(raw); if (Array.isArray(parsed) && parsed.every((item) => item && typeof item.key === "string" && typeof item.title === "string" && typeof item.body === "string")) setSections(parsed); else window.localStorage.removeItem(STORAGE_KEY); } catch { window.localStorage.removeItem(STORAGE_KEY); } }, []);
