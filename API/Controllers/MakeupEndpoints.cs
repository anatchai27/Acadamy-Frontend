...
public static class MakeupEndpoints
{
    public static IEndpointRouteBuilder MapMakeupEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/credits", async (IMakeupService service, HttpContext context, int? student_id, string? status, CancellationToken ct) =>
            await Execute(() => service.ListCreditsAsync(student_id, ActorId(context), context.User.IsInRole("parent"), status, ct), Results.Ok));
        group.MapGet("/bookings", async (int student_id, IMakeupService service, HttpContext context, CancellationToken ct) =>
            await Execute(() => service.ListBookingsAsync(student_id, ActorId(context), context.User.IsInRole("parent"), ct), Results.Ok));
