...
const formDefaults = {
  institute_name: '', contact_phone: '', admin_full_name: '', line_id: '', admin_email: '', admin_phone: '', admin_password: '', consent_version: '',
};
...
export function RegisterPage() {
  ...
  const { control, handleSubmit, watch, trigger, clearErrors, formState: { errors } } = useForm({ mode: 'onChange', reValidateMode: 'onChange', defaultValues: formDefaults });
  ...
  const handleLogoChange = useCallback((base64, validationError) => {
    if (validationError) { setLogoError(validationError); setLogoBase64(null); }
    else { setLogoError(null); setLogoBase64(base64 || null); }
  }, []);

  const handleStep1Next = async () => {
    const valid = await trigger(['institute_name', 'contact_phone']);
    if (!valid) return;
    if (!instituteName?.trim()) { showToast('กรุณากรอกชื่อสถาบัน', 'error'); return; }
    if (!PHONE_REGEX.test(contactPhone || '')) { showToast('กรุณากรอกเบอร์โทรศัพท์ให้ถูกต้อง (9-10 หลัก)', 'error'); return; }
    if (logoError) { showToast('กรุณาแก้ไขไฟล์โลโก้ให้ถูกต้อง', 'error'); return; }
    setStep(2);
  };

  const handleBack = () => { setStep(1); setServerEmailError(''); };

  const onSubmit = async (data) => {
    if (!canSubmit) return;
    setSubmitting(true); setServerEmailError('');
    try {
      const payload = {
        institute: { name: data.institute_name, contactPhone: data.contact_phone, logoBase64: logoBase64 || undefined },
        admin: { fullName: data.admin_full_name },
        role: 'admin', email: data.admin_email, password: data.admin_password, phone: data.admin_phone, lineUserId: data.line_id, acceptPdpa: true, pdpaConsentVersion: data.consent_version || '1.0',
      };
      ...
      if (error?.status === 400 && (msg.toLowerCase().includes('email') || msg.toLowerCase().includes('อีเมล'))) {
        setServerEmailError('อีเมลนี้มีการใช้งานในระบบแล้ว'); setStep(2);
      } else {
  ...
  return (
    <div class={'min-h-screen flex flex-col md:flex-row ' + (isNeo ? 'bg-[#FAF3E0]' : '')}>
      {/* LEFT: Branding */}
          ...
          <form onSubmit={handleSubmit(onSubmit, onError)} class="flex flex-col gap-5">
            {step === 1 && (
              <>
                ...
                <Controller name="institute_name" control={control} rules={{ required: 'กรุณากรอกชื่อสถาบันสอนพิเศษ', maxLength: { value: INSTITUTE_NAME_MAX, message: 'ชื่อสถาบันต้องไม่เกิน 100 ตัวอักษร' } }} render={({ field }) => (<Input type="text" label="ชื่อสถาบันสอนพิเศษ" id="reg-institute-name" placeholder="เช่น สถาบันกวดวิชา TiwHub Tutor" error={errors.institute_name?.message} value={field.value} onChange={(e) => field.onChange(e.target.value)} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="contact_phone" control={control} rules={{ required: 'กรุณากรอกเบอร์โทรศัพท์สถาบัน', validate: (v) => PHONE_REGEX.test(v) || 'กรุณากรอกเบอร์โทรศัพท์ให้ถูกต้อง 9-10 หลัก' }} render={({ field }) => (<Input type="tel" label="เบอร์โทรศัพท์สถาบัน" id="reg-institute-phone" placeholder="02-XXX-XXXX หรือเบอร์มือถือ" error={errors.contact_phone?.message} value={field.value} onChange={(e) => { const val = e.target.value.replace(/\D/g, ''); field.onChange(val.slice(0, 10)); }} onBlur={field.onBlur} name={field.name} />)} />
                <Button variant="primary" size="lg" type="button" disabled={!canGoToStep2} onClick={handleStep1Next}>ถัดไป</Button>
              </>
            )}
            {step === 2 && (
              <>
                <Controller name="admin_full_name" control={control} rules={{ required: 'กรุณากรอกชื่อ-นามสกุล', validate: (v) => v.trim().length > 0 || 'กรุณากรอกชื่อ-นามสกุล' }} render={({ field }) => (<Input type="text" label="ชื่อ-นามสกุล" id="reg-full-name" placeholder="กรอกชื่อ-นามสกุล" error={errors.admin_full_name?.message} value={field.value} onChange={(e) => field.onChange(e.target.value)} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="line_id" control={control} rules={{ required: 'กรุณากรอก Line ID', validate: (v) => v.trim().length > 0 || 'กรุณากรอก Line ID' }} render={({ field }) => (<Input type="text" label="Line ID" id="reg-line-id" placeholder="กรอก Line ID" error={errors.line_id?.message} value={field.value} onChange={(e) => field.onChange(e.target.value)} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="admin_email" control={control} rules={{ required: 'กรุณากรอกอีเมล', validate: (v) => EMAIL_REGEX.test(v) || 'กรุณากรอกรูปแบบอีเมลให้ถูกต้อง' }} render={({ field }) => (<Input type="email" label="อีเมลสำหรับเข้าสู่ระบบ" id="reg-email" placeholder="กรอกอีเมล" error={emailDisplayError} value={field.value} onChange={(e) => { field.onChange(e.target.value); setServerEmailError(''); if (errors.admin_email?.type === 'manual') clearErrors('admin_email'); }} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="admin_phone" control={control} rules={{ required: 'กรุณากรอกเบอร์โทรศัพท์', validate: (v) => v.trim().length > 0 || 'กรุณากรอกเบอร์โทรศัพท์' }} render={({ field }) => (<Input type="tel" label="เบอร์โทรศัพท์" id="reg-phone" placeholder="กรอกเบอร์โทรศัพท์" error={errors.admin_phone?.message} value={field.value} onChange={(e) => field.onChange(e.target.value)} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="admin_password" control={control} rules={{ required: 'กรุณากรอกรหัสผ่าน', validate: (v) => PASSWORD_REGEX.test(v) || 'รหัสผ่านต้องมีความยาวอย่างน้อย 8 ตัว ตัวพิมพ์เล็ก พิมพ์ใหญ่ และตัวเลข' }} render={({ field }) => (<Input type="password" label="รหัสผ่าน" id="reg-password" placeholder="ตั้งรหัสผ่าน" error={errors.admin_password?.message} value={field.value} onChange={(e) => field.onChange(e.target.value)} onBlur={field.onBlur} name={field.name} />)} />
                <Controller name="consent_version" control={control} rules={{ required: 'กรุณายอมรับข้อตกลงและเงื่อนไขการใช้งาน', validate: (v) => v.trim().length > 0 || 'กรุณายอมรับข้อตกลงและเงื่อนไขการใช้งาน' }} render={({ field }) => (<div class="flex flex-col gap-1"><Checkbox id="consent" checked={!!field.value} onChange={(e) => field.onChange(e.target.checked ? 'v1.0' : '')} onBlur={field.onBlur} label="ฉันยอมรับข้อตกลงและเงื่อนไขการใช้งาน (PDPA)" error={errors.consent_version?.message} required name={field.name} /><a href="/privacy" target="_blank" rel="noopener noreferrer" class="text-xs text-oasis-primary underline hover:no-underline ml-6">อ่านนโยบายความเป็นส่วนตัว</a></div>)} />
