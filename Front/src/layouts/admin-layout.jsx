...
const getPageTitle = path => {
  const labels = menuGroups
    .map(group => group.items.filter(m => m.path === path).map(m => m.label))
    .filter(arr => arr.length > 0);
  return labels.length > 0 ? labels[0][0] : 'หน้าหลัก';
};
export const AdminLayout = ({
  ...
  useEffect(() => {
    return currentPath ? (() => {
      ...
      return allPaths.every(p => stored.includes('page_' + p.replace(/\//g, '_'))) ? (() => {
  ...
  return <div class={`min-h-screen ${isNeo ? 'bg-[#FAF3E0] text-black' : 'bg-slate-50 text-slate-700'}`}>
      <aside class={`hidden md:fixed md:inset-y-0 md:left-0 md:flex md:w-64 md:flex-col md:z-20 ${isNeo ? 'bg-[#FFF] border-r-3 border-black' : 'bg-white border-r border-slate-200 shadow-sm'}`}>
        ...
        <nav class="flex-1 space-y-0.5 px-3 py-5 overflow-y-auto">
          {filteredGroups.map(group => <div key={group.label} class="mb-4">
            ...
            return <button key={item.path} onClick={() => route(item.path)} class={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${isActive ? activeBg : inactiveBg}`}>
                    <item.icon class={`h-5 w-5 shrink-0 ${isActive ? activeIconColor : inactiveIconColor}`} />
                    {item.label}
                    {isActive ? <div class={`ml-auto w-1 h-5 rounded-full ${isNeo ? 'bg-white' : 'bg-blue-600'}`} /> : null}
                  </button>;
          })}
            </div>)}
        </nav>
        ...
        <div class={`${isNeo ? 'border-t-3 border-black' : 'border-t border-slate-100'} p-4`}>
          ...
          <button onClick={handleLogout} class={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${isNeo ? 'border-2 border-black text-black hover:bg-red-500 hover:text-white hover:border-red-500' : 'text-zinc-600 hover:text-red-600 hover:bg-red-50'}`}>
      ...
      <header class={`md:hidden fixed top-0 left-0 right-0 z-20 h-14 flex items-center justify-between px-4 shadow-sm ${isNeo ? 'bg-[#FFF] border-b-3 border-black' : 'bg-white/80 backdrop-blur-lg border-b border-slate-200'}`}>
        ...
        <div class="relative">
          ...
          {dropdownOpen ? <div class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-200 py-1 z-30">
              ...
              <button onClick={handleLogout} class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left text-zinc-600 hover:bg-red-50 hover:text-red-600 transition-colors">
      ...
      <header class={`hidden md:sticky md:top-0 md:z-10 md:ml-64 md:flex md:h-16 md:items-center md:justify-between md:px-8 ${isNeo ? 'bg-[#FFF] border-b-3 border-black' : 'bg-white/80 backdrop-blur-lg border-b border-slate-200 shadow-sm'}`}>
        <div class="flex items-center gap-4">
          ...
          <button onClick={() => route('/admin/attendance')} class={`px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center gap-2 ${isNeo ? 'border-3 border-black text-black bg-white shadow-[3px_3px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[1px_1px_0px_#000]' : 'text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-sm hover:shadow-md active:scale-[0.98]'}`}>
      ...
      <nav class={`md:hidden fixed bottom-0 left-0 right-0 z-20 h-16 safe-area-bottom shadow-sm ${isNeo ? 'bg-[#FFF] border-t-3 border-black' : 'bg-white/80 backdrop-blur-lg border-t border-slate-200'}`}>
        <div class="flex items-center overflow-x-auto overflow-y-hidden h-full px-2 gap-1 no-scrollbar">
          ...
          const isActive = currentPath === item.path;
          const btnClass = isActive ? isNeo ? 'text-black bg-black/5' : 'text-blue-600 bg-blue-50' : 'text-slate-400';
          return <button key={item.path} onClick={() => route(item.path)} class={`flex flex-col items-center justify-center gap-0.5 px-2 py-1 min-w-[60px] shrink-0 transition-colors rounded-lg ${btnClass}`}>
