...
export function LeadsPage({ path }) {
  ...
  useEffect(() => { loadLeads(); getUsers({ limit: 200 }).then(response => { const payload = response.data?.data || response.data || {}; setUsers(payload.users || (Array.isArray(payload) ? payload : [])); }).catch(() => setUsers([])); }, [status]);
  ...
  const saveFollowUp = async (lead, nextStatus, notes) => {
    ...
    try {
      ...
      setLeads(current => current.map(item => item.id === lead.id ? { ...item, status: nextStatus, notes, assignedTo: lead.assignedTo || null } : item));
  ...
  return <AdminLayout path={path}><div class="mx-auto max-w-7xl px-4 py-6 sm:px-6">
    ...
    <div class="space-y-3">{leads.map(lead => <LeadCard key={lead.id} lead={lead} users={users} saving={savingId === lead.id} onSave={saveFollowUp} />)}</div>
  </div></AdminLayout>;
}

function LeadCard({ lead, users, saving, onSave }) {
  const [nextStatus, setNextStatus] = useState(lead.status);
  const [notes, setNotes] = useState(lead.notes || '');
  const [assignedTo, setAssignedTo] = useState(lead.assignedTo || '');
  return <article class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm"><div class="flex flex-wrap items-start justify-between gap-4"><div><h2 class="font-semibold text-zinc-900">{lead.fullName}</h2><p class="mt-1 text-sm text-zinc-500">{lead.phone}{lead.email ? ` · ${lead.email}` : ''}</p>{lead.studentName && <p class="mt-1 text-xs text-zinc-500">นักเรียน: {lead.studentName}</p>}</div><span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{statuses.find(([value]) => value === lead.status)?.[1] || lead.status}</span></div><div class="mt-4 grid gap-3 md:grid-cols-[180px_180px_1fr_auto]"><select value={nextStatus} onChange={event => setNextStatus(event.target.value)} class="rounded-xl border border-zinc-200 px-3 py-2 text-sm">{statuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={assignedTo} onChange={event => setAssignedTo(event.target.value)} class="rounded-xl border border-zinc-200 px-3 py-2 text-sm"><option value="">ยังไม่มอบหมาย</option>{users.map(user => <option key={user.id} value={user.id}>{user.fullName || user.email}</option>)}</select><textarea value={notes} onInput={event => setNotes(event.target.value)} placeholder="บันทึกการติดตาม" rows="2" class="rounded-xl border border-zinc-200 px-3 py-2 text-sm" /><Button variant="primary" size="sm" loading={saving} disabled={saving} onClick={() => onSave({ ...lead, assignedTo: assignedTo ? Number(assignedTo) : null }, nextStatus, notes)}>บันทึก follow-up</Button></div></article>;
}
