...
const greetings = [
  ...
  'วันนี้อากาศดี เหมาะแก่การจัดการนักเรียนเป็นอย่างยิ่ง 🌤️',
  'สวัสดี! TiwHub ดีใจที่ได้เห็นคุณอีกครั้ง 🙌',
  'TIP: ถ้าเหนื่อย ให้กด \'สแกน QR\' แล้วยืดเส้นยืดสาย',
  'Admin คนนี้ทำอะไรเก่งไปหมดเลย!',
  'วันนี้มีอะไรให้ช่วยจัดการบ้าง? บอก TiwHub มาได้เลย!',
];

function getGreetingForToday() {
  try {
    const day = new Date().toDateString();
    const stored = localStorage.getItem('th_greeting_day');
    const storedIdx = localStorage.getItem('th_greeting_idx');
    if (stored === day && storedIdx !== null) return greetings[Number(storedIdx)];
...
export function PlayfulGreeting({ userName }) {
  ...
  return (
    <div class={isNeo ? 'neo-card bg-[#FFEAA7] p-5 mb-8' : 'bg-gradient-to-r from-oasis-primary/5 to-oasis-accent/5 border border-oasis-primary/10 rounded-2xl p-5 mb-8'}>
