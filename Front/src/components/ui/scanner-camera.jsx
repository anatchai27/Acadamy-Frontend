...
export const ScannerCamera = ({
  ...
  const scanFrame = () => {
    return !scanningRef.current ? undefined : (() => {
      const video = videoRef.current;
      const canvas = canvasRef.current;
      !video || !canvas || video.readyState < 2 ? (() => {
      ...
      code && code.data !== lastScanRef.current ? (() => {
  ...
  return <div class="flex flex-col gap-4">
      <div class="relative w-full aspect-square max-w-md mx-auto bg-zinc-800 rounded-2xl overflow-hidden flex items-center justify-center">
        <video ref={videoRef} class={`w-full h-full object-cover ${cameraReady ? 'block' : 'hidden'}`} muted playsinline />
        <canvas ref={canvasRef} class="hidden" />

        {!cameraReady && !cameraError ? <div class="text-center text-zinc-400 px-6"><HiOutlineCamera class="h-12 w-12 mx-auto mb-3 text-oasis-primary animate-pulse" />
            <p class="text-sm">กำลังเปิดกล้อง...</p>
          </div> : null}

        {cameraError ? <div class="text-center text-zinc-400 px-6"><HiOutlineCamera class="h-12 w-12 mx-auto mb-3 text-oasis-primary" />
      ...
      <form onSubmit={handleSubmit} class="flex gap-2 max-w-md mx-auto w-full">
        <input type="text" value={manualValue} onInput={e => setManualValue(e.target.value)} placeholder="หรือพิมพ์รหัสนักเรียน" class="flex-1 px-4 py-2.5 bg-white border border-zinc-200 rounded-xl text-sm focus:outline-none focus:border-oasis-primary focus:ring-2 focus:ring-oasis-primary/10 text-zinc-800 placeholder:text-zinc-400" />
        <button type="submit" class="px-5 py-2.5 bg-oasis-primary hover:bg-oasis-primary-dark text-white text-sm font-medium rounded-xl transition-colors shadow-sm">
          ยืนยัน
        </button>
      </form>
    </div>;
