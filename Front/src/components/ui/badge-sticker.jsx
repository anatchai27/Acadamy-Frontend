...
const storeBadges = badges => {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(badges));
};
export const unlockBadge = id => {
  const badges = getStoredBadges();
  return !badges.includes(id) && BADGE_DATA[id] ? (() => {
    badges.push(id);
    storeBadges(badges);
    window.dispatchEvent(new CustomEvent('th-badge-unlock', {
      detail: {
        id
      }
    }));
