...
export const LogoUpload = ({
  ...
  return <div class="flex flex-col gap-1.5">
      ...
      <div class={`relative w-40 h-40 border-2 border-dashed rounded-xl flex items-center justify-center cursor-pointer transition-colors ${error ? 'border-red-400 bg-red-50' : isDragging ? 'border-oasis-primary bg-oasis-primary/5' : 'border-zinc-200 bg-white hover:border-zinc-300'}`} onClick={() => inputRef.current?.click()} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
        {preview ? <>
            <img src={preview} alt="โลโก้สถาบัน" class="w-full h-full object-cover rounded-xl" />
            <button type="button" class="absolute -top-2 -right-2 bg-oasis-danger text-white rounded-full p-1.5 hover:bg-oasis-danger-dark transition-colors shadow-sm" onClick={handleRemove} title="ลบรูปภาพ">
              <TrashIcon />
            </button>
