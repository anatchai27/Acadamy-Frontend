...
import { apiErrorMessage, validateLeaveAttachment } from '../utils/validation';
...
const LeavePanel = ({ childId, refreshKey, onRefresh }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([getChildSessions(childId), getChildLeaveRequests(childId)])
      ...
      .catch(apiError => active && setError(apiErrorMessage(apiError, 'โหลดข้อมูลการลาไม่สำเร็จ')))
  ...
  const submit = async event => {
    ...
    setError('');
    setSuccess('');
    const attachmentError = validateLeaveAttachment(attachment);
    if (attachmentError) {
    ...
    try {
      ...
      setError(apiErrorMessage(apiError, 'ส่งคำขอลาไม่สำเร็จ'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <section class="rounded-card bg-white p-5 shadow-soft">
        ...
        <form class="mt-5 space-y-4" onSubmit={submit}>
          ...
          <label class="block"><span class="mb-1.5 block text-sm font-bold text-ink-700">เหตุผล</span><textarea value={reason} onInput={event => setReason(event.currentTarget.value)} rows="3" placeholder="เช่น ป่วย มีธุระครอบครัว" class="w-full resize-none rounded-2xl border border-sage-200 bg-sage-50 px-3 py-3 text-sm text-ink-900 outline-none placeholder:text-ink-500 focus:border-sage-600 focus:ring-2 focus:ring-sage-200" disabled={submitting} /></label>
           <label class="block"><span class="mb-1.5 block text-sm font-bold text-ink-700">หลักฐานการลา <span class="font-normal text-ink-500">(ถ้ามี, PDF/JPG/PNG/WEBP ไม่เกิน 5MB)</span></span><input aria-label="หลักฐานการลา" type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={event => { const file = event.currentTarget.files?.[0] || null; setAttachment(file); setError(validateLeaveAttachment(file)); }} disabled={submitting} class="block w-full rounded-2xl border border-dashed border-sage-300 bg-sage-50 px-3 py-3 text-sm text-ink-700 file:mr-3 file:rounded-xl file:border-0 file:bg-sage-600 file:px-3 file:py-2 file:font-bold file:text-white" /></label>
...
const MakeupPanel = ({ childId, refreshKey, onRefresh }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([getMakeupCredits(childId), getMakeupBookings(childId), getMakeupSlots({ from: new Date().toISOString() })])
      ...
      .catch(apiError => active && setError(apiErrorMessage(apiError, 'โหลดสิทธิ์เรียนชดเชยไม่สำเร็จ')))
      .finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [childId, refreshKey]);
  ...
  const book = async slot => {
    ...
    if (slot.status !== 'open' || slot.bookedCount >= slot.capacity) { setError('ที่นั่งนี้เต็มหรือไม่เปิดรับจองแล้ว'); return; }
    ...
    try {
      ...
      setError(apiErrorMessage(apiError, 'จองที่นั่งไม่สำเร็จ อาจมีผู้จองเต็มแล้ว'));
  ...
  const cancel = async booking => {
    ...
    catch (apiError) { setError(apiErrorMessage(apiError, 'ยกเลิกการจองไม่สำเร็จ')); }
    finally { setBookingId(null); }
  };
  ...
  return (
    <div class="space-y-4">
      ...
      <section><SectionTitle icon={HiOutlineTicket} title="การจองของฉัน" /><div class="mt-3 space-y-2">{loading ? <SkeletonList /> : bookings.length ? bookings.map(booking => <BookingItem key={booking.id} booking={booking} busy={bookingId === booking.id} onCancel={() => cancel(booking)} />) : <EmptyState text="ยังไม่มีการจองเรียนชดเชย" />}</div></section>
       <section><SectionTitle icon={HiOutlineCalendarDays} title="ที่นั่งเรียนชดเชยที่เปิดอยู่" /><div class="mt-3 space-y-3">{loading ? <SkeletonList /> : slots.length ? slots.map(slot => <SlotItem key={slot.id} slot={slot} disabled={!availableCredit || slot.status !== 'open' || slot.bookedCount >= slot.capacity || bookingId === slot.id} busy={bookingId === slot.id} onBook={() => book(slot)} onCancel={() => cancel(slot)} />) : <EmptyState text="ยังไม่มีที่นั่งเรียนชดเชย" />}</div></section>
    </div>
  );
};
...
const SlotItem = ({ slot, disabled, busy, onBook, onCancel }) => <article class="rounded-2xl border border-sage-100 bg-white p-4 shadow-soft"><div class="flex items-start justify-between gap-3"><div><p class="font-bold text-ink-900">ครูผู้สอน #{slot.teacherId}</p><p class="mt-1 text-sm text-ink-500">{formatDate(slot.scheduledAt)} · ห้อง {slot.roomId || '-'}</p></div><span class="rounded-full bg-gold-50 px-2.5 py-1 text-xs font-bold text-gold-600">{slot.bookedCount}/{slot.capacity} ที่นั่ง</span></div><div class="mt-3 flex gap-2"><button type="button" disabled={disabled} onClick={onBook} class="min-h-10 flex-1 rounded-xl bg-sage-600 px-3 text-sm font-bold text-white hover:bg-sage-700 disabled:cursor-not-allowed disabled:opacity-50">{busy ? 'กำลังดำเนินการ...' : slot.bookedCount >= slot.capacity ? 'เต็มแล้ว' : 'จองที่นั่ง'}</button>{slot.status === 'reserved' && <button type="button" disabled={busy} onClick={onCancel} class="min-h-10 rounded-xl border border-sage-200 px-3 text-sm font-bold text-sage-700 disabled:opacity-50">ยกเลิก</button>}</div></article>;
