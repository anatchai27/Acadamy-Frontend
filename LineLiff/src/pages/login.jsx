...
import { bindLineUserId, getParentDashboard } from '../services/parent-service';
import { getLiffAccessToken } from '../services/liff';
...
export function LoginPage() {
  ...
  useEffect(() => {
    if (!state.liffInitialized) return;
    if (!state.liffProfile) {
      (async () => {
        const { getLiffProfile } = await import('../services/liff');
        const profile = await getLiffProfile();
        if (profile) dispatch({ type: 'SET_LIFF_PROFILE', payload: profile });
      })();
      return;
    }
    if (state.parentUser) {
    ...
    (async () => {
      try {
        ...
        if (token) {
          try { window.localStorage.setItem('parent_token', token); } catch {}
          dispatch({ type: 'SET_PARENT_TOKEN', payload: token });
        }
